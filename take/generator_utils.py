...
def seq_get_or_generate_vocab(data_dir, tmp_dir,
                          vocab_filename, vocab_size, sources=None):
  """Generate a vocabulary from the datasets in sources (_DATA_FILE_URLS)."""
  vocab_filepath = os.path.join(data_dir, vocab_filename)
  token_counts = defaultdict(int)
  tf.logging.info("Generating vocab")

  with tf.gfile.GFile('t2t_split_data/input.txt', mode="r") as source_file:
  ...
  with tf.gfile.GFile('t2t_split_data/target.txt', mode="r") as source_file:
    file_byte_budget = 3.5e5
    for line in source_file:
      if file_byte_budget <= 0:
        break
